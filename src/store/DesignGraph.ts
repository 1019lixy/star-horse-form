...
export const useDesignGraphStore = defineStore(
  ...
  () => {
    ...
    const addNode = (data: NodeInfo): Cell | null => {
      ...
      cell.setAttrByPath("use", {
        "xlink:href": data.icon ? `#icon-${data.icon}` : "#icon-default",
      });
      return cell;
    };
    /**
    ...
    const addEdge = (edge: EdgeInfo) => {
      ...
      const sourcePoint =
        edge.sourcePoint ||
        sourcePorts.find((item: any) => item.group == group.source)?.id;
      const targetPoint =
        edge.targetPoint ||
        targetPorts.find((item: any) => item.group == group.target)?.id;
      const datat = {
