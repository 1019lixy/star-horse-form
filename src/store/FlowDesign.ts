...
import {apiInstance, ApiUrls, postRequest, SelectOption,} from "star-horse-lowcode";
import {nodePrepList} from "@/views/workflow/plugin/utils/nodePreps";
import {ProcessVariablesType} from "@/components/types/FlowDesignType.js";

const dynamicFormUrl: ApiUrls = apiInstance(
    "userdb-manage",
    "userdb/dynamicForm",
);
...
export const useFlowDesignStore = defineStore("flowDesign", () => {
    ...
    const formColumns = ref<SelectOption[]>([]);
    // 流程变量
    const processVariables = ref<ProcessVariablesType[]>([]);
    const getProcessVariables = () => {
        return processVariables.value;
    };
    const setProcessVariables = (variables: ProcessVariablesType[]) => {
     ...
     * @param id
     */
    const loadDynamicForm = (id: string) => {
        postRequest(
            dynamicFormUrl.basePrefix + "/analysisMainTableFieldsByIds",
            Array.isArray(id) ? id : [id],
        ).then((response: any) => {
            formColumns.value = response.data.data;
        });
    };
    ...
    const flowAddNode = (
        ...
        if (parentNodeType == FlowNodeEnums.START_NODE) {
            //  开始
            if (Object.prototype.hasOwnProperty.call(parentNode, "name")) {
                // 如果添加的是并行节点
    ...
    const flowDelNode = (snode: any) => {
        if (snode.id == node.value.id) {
            ...
            snode.type == FlowNodeEnums.BRANCH_CONDITION_NODE ||
     ...
     * 更新节点
     */
    const flowUpdateNode = ({currNode, field, value}: { currNode: any; field: any; value: any }) => {
    ...
    /**
     * 初始化
     * @param stopInitFormInfo 是否初始化表单信息
     */
    const init = (stopInitFormInfo?: boolean) => {
        refreshMap(true);
        nodeList.value = [];
        nodePrepMap.value = {};
        parentNode.value = {};
