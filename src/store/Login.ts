...
export const useLoginStore = defineStore("login", () => {
    ...
    const loadMenusInfo = (userId: string, appId: string) => {
        ...
        postRequest(`${menuUrl.basePrefix}/permissionMenus/${userId}/${appId}`, []).then((res: any) => {
    ...
    const loadAppInfo = (userId: string) => {
        postRequest(`${appinfoUrl.basePrefix}/getUserSystem/${userId}`, []).then((res: any) => {
            const redata = res?.data?.data;
    ...
    const loadShortMenu = () => {
        let param: any = [];
        postRequest(`${shortcutMenuUrl.basePrefix}/currentUserShortcut`, param).then((res: any) => {
            const redata = res?.data?.data;
 ...
 */
    const userLogin = async (loginData: any) => {
        ...
        let data: any = {};
        let errMsg = null;
        const resultData = await loadData(`${userAuditUrl.basePrefix}/userLogin`, loginData);
        if (resultData.error) {
