...
export const useFlexDesignStore = defineStore("flexDesign", () => {
   ...
   * @returns 序列化的设计数据
   */
  const serializeDesignData = (name: string, description: string = "", flexModel: string = "flex") => {
    return {
      name,
      description,
  ...
  const getDesignSummary = () => {
    return {
      ...
      compCount: Object.keys(compList.value).reduce((total, key) => total + compList.value[key].length, 0),
      containerDirection: containerDirection.value,
   ...
   * @returns HTML代码字符串
   */
  const generatePreviewHTML = (flexModel: string = "flex", containerDataForm?: any) => {
    ...
    if (flexModel === 'grid') {
      ...
      if (!containerData.gridTemplateColumns && !containerData.gridTemplateRows) {
