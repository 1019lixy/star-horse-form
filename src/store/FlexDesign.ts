...
export const useFlexDesignStore = defineStore("flexDesign", () => {
   ...
   */
  const setContainerContent = (containerContent: any) => {
    if (!containerInfo.value.containerContent) {
      containerInfo.value.containerContent = {};
    }
    containerInfo.value.containerContent = containerContent;
  };
  /**
   * 设置容器方向
   * @param direction 方向
