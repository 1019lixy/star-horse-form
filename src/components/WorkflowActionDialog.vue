...
const formRules = computed(() => {
  ...
  if (props.actionType === "b" || props.actionType === "n") {
    rules.rejectReason = [
      {required: true, message: i18n("workflow.action.rejectReason.required"), trigger: "blur"}
    ];
  }

  if (props.actionType === "c" || props.actionType === "d" || props.actionType === "e") {
    rules.returnReason = [
      {required: true, message: i18n("workflow.action.returnReason.required"), trigger: "blur"}
    ];
  }

  if (props.actionType === "h") {
    rules.addSignUsers = [
      {required: true, message: i18n("workflow.action.addSignUsers.required"), trigger: "change"}
    ];
  }

  if (props.actionType === "i") {
    rules.reduceSignUsers = [
      {required: true, message: i18n("workflow.action.reduceSignUsers.required"), trigger: "change"}
    ];
  }

  if (props.actionType === "g") {
    rules.transferTo = [
      {required: true, message: i18n("workflow.action.transferTo.required"), trigger: "change"}
    ];
  }

  if (props.actionType === "p") {
    rules.assignApprover = [
      {required: true, message: i18n("workflow.action.assignApprover.required"), trigger: "change"}
    ];
  }
...
const showComments = computed(() => {
  return ["a", "b", "c", "d", "e", "g", "h", "i", "j", "l", "m", "n", "o", "p", "q"].includes(props.actionType);
});
