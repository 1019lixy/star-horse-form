 ...
 * @param isSelf
 */
export const createMessage = (sender: any, content: string, isSelf: boolean = false): ChatMessage => {
    return {
        id: uuid(),
...
export const useChatManager = (options: ChatManagerOptions) => {
    ...
    const handleIncomingChatMessage = (message: any) => {
        ...
        chatMessages.value.push(createMessage(message,message.content,isSelf));
        scrollToLatestMessage();
    };
