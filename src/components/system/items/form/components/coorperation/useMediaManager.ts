  ...
  const initializeMediaManagers = () => {
    ...
    screenShareManager.value = new ScreenShareManager({
      onStreamAcquired: (stream) => {
        ...
        // 立即触发WebRTC重新协商，确保其他参与者能看到屏幕共享
        if (window.screenShareRenegotiationCallback) {
          window.screenShareRenegotiationCallback(true);
        }
      },
      onStreamStopped: () => {
        ...
        // 触发WebRTC重新协商
        if (window.screenShareRenegotiationCallback) {
          window.screenShareRenegotiationCallback(false);
        }
      }
    });
