import {CompType} from "star-horse-lowcode";

export const formItems: CompType[] = [
    {
        "itemName": "单行文本框",
        "itemType": "input",
        "itemIcon": "input",
        "category": 1,
        "fields": [],
        "advancedFields": [
            {
                "label": "原生 autocomplete 属性",
                "fieldName": "autocomplete",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示统计字数",
                "fieldName": "showWordLimit",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "缩放",
                "fieldName": "resize",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "禁止缩放", "value": "none"}, {
                    "name": "垂直和水平缩放",
                    "value": "both"
                }, {"name": "垂直缩放", "value": "vertical"}, {"name": "水平缩放", "value": "horizontal"}],
                "defaultValues": "none",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最小输入长度",
                "fieldName": "minlength",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "设置最大值",
                "fieldName": "max",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "设置最小值",
                "fieldName": "min",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "指定输入值的格式",
                "fieldName": "formatter",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "输入框行数",
                "fieldName": "rows",
                "fieldType": "number",
                "required": false,
                "selectValues": "2",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "在 Input 值改变时触发",
                "actionName": "input",
                "fieldType": "button"
            },
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {

                "label": "Clearable 属性生成的清空按钮时触发",
                "actionName": "clear",
                "fieldType": "button"
            },
            {

                "label": "在 Input 失去焦点时触发",
                "actionName": "blur",
                "fieldType": "button"
            },
            {

                "label": "在 Input 获得焦点时触发",
                "actionName": "focus",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "数字输入框",
        "itemType": "number",
        "itemIcon": "number",
        "category": 1,
        "fields": [
            {
                "label": "设置最大值",
                "fieldName": "max",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "计数器步长",
                "fieldName": "step",
                "fieldType": "input",
                "required": false,
                "selectValues": "1",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "数值精度",
                "fieldName": "precision",
                "fieldType": "number",
                "required": false,
                "selectValues": "0",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "设置最小值",
                "fieldName": "min",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "清空时显示的值",
                "fieldName": "valueOnClear",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "最小值", "value": "min"}, {"name": "最大值", "value": "max"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否使用控制按钮",
                "fieldName": "controls",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "按钮位置",
                "fieldName": "controlsPosition",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "左", "value": "left"}, {"name": "右", "value": "right"}],
                "defaultValues": "right",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {
                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {
                "label": "在 Input 失去焦点时触发",
                "actionName": "blur",
                "fieldType": "button"
            },
            {
                "label": "在 Input 获得焦点时触发",
                "actionName": "focus",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "选择器",
        "itemType": "select",
        "itemIcon": "select",
        "category": 1,
        "fields": [
            {
                "label": "显示所有选中标签",
                "fieldName": "collapseTagsTooltip",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "多选时按文字形式展示",
                "fieldName": "collapseTags",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "是否可筛选",
                "fieldName": "filterable",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "是否多选",
                "fieldName": "multiple",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否保留搜索关键词",
                "fieldName": "reserveKeyword",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "否从服务器远程加载",
                "fieldName": "remote",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否正在从远程获取数据",
                "fieldName": "loading",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "显示下拉箭头",
                "fieldName": "showArrow",
                "fieldType": "switch",
                "required": false,
                "selectValues": "",
                "defaultValues": "true",
                "category": 2,
                "configParams": []
            },
            {
                "label": "标签类型",
                "fieldName": "placement",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "顶部", "value": "top"}, {
                    "name": "顶部开始",
                    "value": "top-start"
                }, {"name": "顶部结束", "value": "top-end"}, {"name": "底部", "value": "bottom"}, {
                    "name": "底部开始",
                    "value": "bottom-start"
                }, {"name": "底部结束", "value": "bottom-end"}, {"name": "左边", "value": "left"}, {
                    "name": "左上",
                    "value": "left-start"
                }, {"name": "左下", "value": "left-end"}, {"name": "右边", "value": "right"}, {
                    "name": "右上",
                    "value": "right-start"
                }, {"name": "右下", "value": "right-end"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "Tooltip 主题",
                "fieldName": "effect",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "暗黑", "value": "dark"}, {"name": "高亮", "value": "light"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义远程搜索",
                "fieldName": "remoteMethod",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "无数据显示的文字",
                "fieldName": "noDataText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "唯一标识KEY",
                "fieldName": "valueKey",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最多可以选择的项目数",
                "fieldName": "multipleLimit",
                "fieldType": "number",
                "required": false,
                "selectValues": "0",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "从服务器加载内容时显示的文本",
                "fieldName": "loadingText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义筛选方法",
                "fieldName": "filterMethod",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义类名",
                "fieldName": "popperClass",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "无匹配时显示的文字",
                "fieldName": "noMatchText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "标签类型",
                "fieldName": "tagType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "成功", "value": "success"}, {
                    "name": "信息",
                    "value": "info"
                }, {"name": "警告", "value": "warning"}, {"name": "危险", "value": "danger"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否允许用户创建新条目",
                "fieldName": "allowCreate",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "默认选择第一个匹配项",
                "fieldName": "defaultFirstOption",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "获得焦点弹出选项菜单",
                "fieldName": "automaticDropdown",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "获得焦点弹出选项菜单",
                "fieldName": "fitInputWidth",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否自动完成",
                "fieldName": "autocomplete",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "下拉框出现/隐藏时触发",
                "actionName": "visibleChange",
                "fieldType": "button"
            },
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {

                "label": "点击清空按钮时触发",
                "actionName": "clear",
                "fieldType": "button"
            },
            {

                "label": "在 Input 失去焦点时触发",
                "actionName": "blur",
                "fieldType": "button"
            },
            {

                "label": "在 Input 获得焦点时触发",
                "actionName": "focus",
                "fieldType": "button"
            },
            {

                "label": "多选模式下移除tag时触发",
                "actionName": "removeTag",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "日期选择器",
        "itemType": "datetime",
        "itemIcon": "datetime",
        "category": 1,
        "fields": [
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "文本框可输入",
                "fieldName": "editable",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "true",
                "category": 1,
                "configParams": []
            },
            {
                "label": "日期格式",
                "fieldName": "format",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "年月日时分秒", "value": "YYYY-MM-DD HH:mm:ss"}, {
                    "name": "年月日",
                    "value": "YYYY-MM-DD"
                }, {"name": "年月日(短)", "value": "Y-M-D"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "显示类型",
                "fieldName": "type",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "年", "value": "year"}, {"name": "月", "value": "month"}, {
                    "name": "周",
                    "value": "week"
                }, {"name": "日期", "value": "date"}, {
                    "name": "日期和时间",
                    "value": "datetime"
                }, {"name": "日期和时间范围", "value": "datetimerange"}, {"name": "日期范围", "value": "daterange"}],
                "defaultValues": "date",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否用箭头按钮",
                "fieldName": "timeArrowControl",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位内容A",
                "fieldName": "startPlaceholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位内容B",
                "fieldName": "endPlaceholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "范围分隔符",
                "fieldName": "rangeSeparator",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "禁用日期",
                "fieldName": "disabledDate",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "下拉框出现/隐藏时触发",
                "actionName": "visibleChange",
                "fieldType": "button"
            },
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {

                "label": "在 Input 失去焦点时触发",
                "actionName": "blur",
                "fieldType": "button"
            },
            {

                "label": "在 Input 获得焦点时触发",
                "actionName": "focus",
                "fieldType": "button"
            },
            {

                "label": "datetimerange时日期改变触发",
                "actionName": "calendarChange",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "开关",
        "itemType": "switch",
        "itemIcon": "switch",
        "category": 1,
        "fields": [
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "关闭时的值",
                "fieldName": "inactiveValue",
                "fieldType": "input",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "打开图标",
                "fieldName": "activeIcon",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "状态时是否触发表单的校验",
                "fieldName": "validateEvent",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示加载中",
                "fieldName": "loading",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "关闭时的颜色",
                "fieldName": "inactiveColor",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "关闭图标",
                "fieldName": "inactiveIcon",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "开启时的值",
                "fieldName": "activeValue",
                "fieldType": "input",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "宽度",
                "fieldName": "width",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "开启时文字描述",
                "fieldName": "activeText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "开启时的颜色",
                "fieldName": "activeColor",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "关闭时文字描述",
                "fieldName": "inactiveText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "switch 状态发生变化时",
                "actionName": "change",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "密码框",
        "itemType": "password",
        "itemIcon": "password",
        "category": 1,
        "fields": [],
        "advancedFields": [
            {
                "label": "缩放",
                "fieldName": "resize",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "禁止缩放", "value": "none"}, {
                    "name": "垂直和水平缩放",
                    "value": "both"
                }, {"name": "垂直缩放", "value": "vertical"}, {"name": "水平缩放", "value": "horizontal"}],
                "defaultValues": "none",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最大值",
                "fieldName": "max",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最小值",
                "fieldName": "min",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "样式",
                "fieldName": "inputStyle",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最小长度",
                "fieldName": "minlength",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示切换密码图标",
                "fieldName": "showPassword",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示统计字数",
                "fieldName": "showWordLimit",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "原生 autocomplete 属性",
                "fieldName": "autocomplete",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "在 Input 值改变时触发",
                "actionName": "input",
                "fieldType": "button"
            },
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {

                "label": "Clearable 属性生成的清空按钮时触发",
                "actionName": "clear",
                "fieldType": "button"
            },
            {

                "label": "在 Input 失去焦点时触发",
                "actionName": "blur",
                "fieldType": "button"
            },
            {

                "label": "在 Input 获得焦点时触发",
                "actionName": "focus",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "标签",
        "itemType": "tag",
        "itemIcon": "text",
        "category": 1,
        "fields": [
            {
                "label": "类型",
                "fieldName": "tagType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "成功", "value": "success"}, {
                    "name": "信息",
                    "value": "info"
                }, {"name": "警告", "value": "warning"}, {"name": "危险", "value": "danger"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "是否表头",
                "fieldName": "headerFlag",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "Tag 的主题",
                "fieldName": "effect",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "暗黑", "value": "dark"}, {"name": "高亮", "value": "light"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否有边框描边",
                "fieldName": "hit",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "背景色",
                "fieldName": "color",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "Tag 是否为圆形",
                "fieldName": "round",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否禁用渐变动画",
                "fieldName": "disableTransitions",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "多行文本框",
        "itemType": "textarea",
        "itemIcon": "textarea",
        "category": 1,
        "fields": [
            {
                "label": "高度自适应",
                "fieldName": "autosize",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "输入框行数",
                "fieldName": "rows",
                "fieldType": "number",
                "required": false,
                "selectValues": "2",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "缩放",
                "fieldName": "resize",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "禁止缩放", "value": "none"}, {
                    "name": "垂直和水平缩放",
                    "value": "both"
                }, {"name": "垂直缩放", "value": "vertical"}, {"name": "水平缩放", "value": "horizontal"}],
                "defaultValues": "none",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "指定输入值的格式",
                "fieldName": "formatter",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "设置最大值",
                "fieldName": "max",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示统计字数",
                "fieldName": "showWordLimit",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "Style",
                "fieldName": "inputStyle",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "设置最小值",
                "fieldName": "min",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最小输入长度",
                "fieldName": "minlength",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "原生 autocomplete 属性",
                "fieldName": "autocomplete",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "在 Input 值改变时触发",
                "actionName": "input",
                "fieldType": "button"
            },
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {

                "label": "Clearable 属性生成的清空按钮时触发",
                "actionName": "clear",
                "fieldType": "button"
            },
            {

                "label": "在 Input 失去焦点时触发",
                "actionName": "blur",
                "fieldType": "button"
            },
            {

                "label": "在 Input 获得焦点时触发",
                "actionName": "focus",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "按钮",
        "itemType": "button",
        "itemIcon": "button",
        "category": 1,
        "fields": [
            {
                "label": "类型",
                "fieldName": "type",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "成功", "value": "success"}, {
                    "name": "信息",
                    "value": "info"
                }, {"name": "警告", "value": "warning"}, {"name": "危险", "value": "danger"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "标签名字",
                "fieldName": "btnLabel",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "图标",
                "fieldName": "icon",
                "fieldType": "icon",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否为链接按钮",
                "fieldName": "link",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自动在两个中文字符之间插入空格",
                "fieldName": "autoInsertSpace",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否为朴素按钮",
                "fieldName": "plain",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示文字按钮背景颜色",
                "fieldName": "bg",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义加载中状态图标组件",
                "fieldName": "loadingIcon",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "原生 type 属性",
                "fieldName": "nativeType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "Button", "value": "button"}, {
                    "name": "Submit",
                    "value": "submit"
                }, {"name": "Reset", "value": "reset"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否为圆形按钮",
                "fieldName": "circle",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否为文字按钮",
                "fieldName": "text",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否为圆角按钮",
                "fieldName": "round",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否为加载中状态",
                "fieldName": "loading",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "在点击按钮时触发",
                "actionName": "click",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "多选框",
        "itemType": "checkbox",
        "itemIcon": "checkbox",
        "category": 1,
        "fields": [
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "输入时校验",
                "fieldName": "validateEvent",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "边框",
                "fieldName": "border",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "当前是否勾选",
                "fieldName": "checked",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "没有选中时的值",
                "fieldName": "falseLabel",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "值改变时触发",
                "actionName": "change",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "单选框",
        "itemType": "radio",
        "itemIcon": "radio",
        "category": 1,
        "fields": [
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "输入时校验",
                "fieldName": "validateEvent",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "填充色",
                "fieldName": "fill",
                "fieldType": "color",
                "required": false,
                "selectValues": "#409EFF",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "激活时的文本颜色",
                "fieldName": "textColor",
                "fieldType": "color",
                "required": false,
                "selectValues": "#ffffff",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "值改变时触发",
                "actionName": "change",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "自动补全输入框",
        "itemType": "autocomplete",
        "itemIcon": "autocomplete",
        "category": 1,
        "fields": [
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "获得焦点显示数据",
                "fieldName": "triggerOnFocus",
                "fieldType": "switch",
                "required": true,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否隐藏加载图标",
                "fieldName": "hideLoading",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "下拉框是否与输入框同宽",
                "fieldName": "fitInputWidth",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "菜单弹出位置",
                "fieldName": "placement",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "顶部", "value": "top"}, {
                    "name": "顶部开始",
                    "value": "top-start"
                }, {"name": "顶部结束", "value": "top-end"}, {"name": "底部", "value": "bottom"}, {
                    "name": "底部开始",
                    "value": "bottom-start"
                }, {"name": "底部结束", "value": "bottom-end"}, {"name": "左边", "value": "left"}, {
                    "name": "左上",
                    "value": "left-start"
                }, {"name": "左下", "value": "left-end"}, {"name": "右边", "value": "right"}, {
                    "name": "右上",
                    "value": "right-start"
                }, {"name": "右下", "value": "right-end"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "下拉列表的类名",
                "fieldName": "popperClass",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "点击选中建议项时触发",
                "actionName": "select",
                "fieldType": "button"
            },
            {

                "label": "在Input值改变时触发",
                "actionName": "change",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "级联选择器",
        "itemType": "cascade",
        "itemIcon": "cascade",
        "category": 1,
        "fields": [
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "多选模式下是否折叠Tag",
                "fieldName": "collapseTags",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "显示完整路径",
                "fieldName": "showAllLevels",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "输入框类型",
                "fieldName": "tagType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "成功", "value": "success"}, {
                    "name": "信息",
                    "value": "info"
                }, {"name": "警告", "value": "warning"}, {"name": "危险", "value": "danger"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否可以被搜索",
                "fieldName": "filterable",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示所有选中的标签",
                "fieldName": "collapseTagsTooltip",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "否触发表单的校验",
                "fieldName": "validateEvent",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义搜索逻辑",
                "fieldName": "filterMethod",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "配置选项",
                "fieldName": "props",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "用于分隔选项的字符",
                "fieldName": "separator",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义类名",
                "fieldName": "popperClass",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "下拉框出现/隐藏时触发",
                "actionName": "visibleChange",
                "fieldType": "button"
            },
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {

                "label": "在 Input 失去焦点时触发",
                "actionName": "blur",
                "fieldType": "button"
            },
            {

                "label": "在 Input 获得焦点时触发",
                "actionName": "focus",
                "fieldType": "button"
            },
            {

                "label": "多选模式下移除tag时触发",
                "actionName": "removeTag",
                "fieldType": "button"
            },
            {

                "label": "当展开节点发生变化时触发",
                "actionName": "expandChange",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "上传",
        "itemType": "upload",
        "itemIcon": "btn-upload",
        "category": 1,
        "fields": [
            {
                "label": "自动上传",
                "fieldName": "autoUpload",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "true",
                "category": 1,
                "configParams": []
            },
            {
                "label": "列表类型",
                "fieldName": "listType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "文本", "value": "text"}, {
                    "name": "图片",
                    "value": "picture"
                }, {"name": "图片卡片", "value": "picture-card"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "保留上传结果",
                "fieldName": "keepResult",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "remark": "数据上传成功后会返回路径、文件名、文件类型、文件大小等信息",
                "configParams": []
            },
            {
                "label": "启用拖拽",
                "fieldName": "drag",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "请求头部",
                "fieldName": "headers",
                "fieldType": "json",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "数量限制",
                "fieldName": "limit",
                "fieldType": "number",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "上传文件类型",
                "fieldName": "accept",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "上传地址",
                "fieldName": "action",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "支持多选文件",
                "fieldName": "multiple",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "附加参数",
                "fieldName": "data",
                "fieldType": "json",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "显示已上传文件",
                "fieldName": "showFileList",
                "fieldType": "input",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "凭证信息",
                "fieldName": "withCredentials",
                "fieldType": "input",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": " 穿梭框",
        "itemType": "transfer",
        "itemIcon": "transfer",
        "category": 1,
        "fields": [
            {
                "label": "左边列表标题",
                "fieldName": "leftTitle",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "数据源的字段别名",
                "fieldName": "props",
                "fieldType": "input",
                "required": false,
                "selectValues": "['待选数据','已选数据']",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "右侧列表标题",
                "fieldName": "rightTitle",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否可搜索",
                "fieldName": "filterable",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "状态时是否触发表单的校验",
                "fieldName": "validateEvent",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "排序策略",
                "fieldName": "targetOrder",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "原始", "value": "original"}, {
                    "name": "追加",
                    "value": "push"
                }, {"name": "头部添加", "value": "unshift"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义搜索方法",
                "fieldName": "filterMethod",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "左侧已勾选的数据集合",
                "fieldName": "leftDefaultChecked",
                "fieldType": "input",
                "required": false,
                "selectValues": "[]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "搜索框占位符",
                "fieldName": "filterPlaceholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "右侧已勾选的数据集合",
                "fieldName": "rightDefaultChecked",
                "fieldType": "input",
                "required": false,
                "selectValues": "[]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "左侧列表元素被用户选中 / 取消选中时触发",
                "actionName": "leftCheckChange",
                "fieldType": "button"
            },
            {

                "label": "右侧列表元素变化时触发",
                "actionName": "change",
                "fieldType": "button"
            },
            {

                "label": "右侧列表元素被用户选中 / 取消选中时触发",
                "actionName": "rightCheckChange",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "滑块",
        "itemType": "slider",
        "itemIcon": "slider",
        "category": 1,
        "fields": [
            {
                "label": "数组类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "垂直模式",
                "fieldName": "vertical",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "步长",
                "fieldName": "step",
                "fieldType": "number",
                "required": false,
                "selectValues": "1",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "最大值",
                "fieldName": "max",
                "fieldType": "number",
                "required": false,
                "selectValues": "100",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "最小值",
                "fieldName": "min",
                "fieldType": "number",
                "required": false,
                "selectValues": "0",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否格式化提示信息",
                "fieldName": "formatTooltip",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示输入框",
                "fieldName": "showInput",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示间断点",
                "fieldName": "showStops",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否开启选择范围",
                "fieldName": "range",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示输入框的控制按钮",
                "fieldName": "showInputControls",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示提示信息",
                "fieldName": "showTooltip",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "屏幕阅读器标签结尾的标记",
                "fieldName": "rangeEndLabel",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "标签位置",
                "fieldName": "placement",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "顶部", "value": "top"}, {
                    "name": "顶部开始",
                    "value": "top-start"
                }, {"name": "顶部结束", "value": "top-end"}, {"name": "底部", "value": "bottom"}, {
                    "name": "底部开始",
                    "value": "bottom-start"
                }, {"name": "底部结束", "value": "bottom-end"}, {"name": "左边", "value": "left"}, {
                    "name": "左上",
                    "value": "left-start"
                }, {"name": "左下", "value": "left-end"}, {"name": "右边", "value": "right"}, {
                    "name": "右上",
                    "value": "right-start"
                }, {"name": "右下", "value": "right-end"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "滑块高度，垂直模式必填",
                "fieldName": "height",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "屏幕阅读器标签开始的标记",
                "fieldName": "rangeStartLabel",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义类名",
                "fieldName": "tooltipClass",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "数据改变时触发",
                "actionName": "input",
                "fieldType": "button"
            },
            {

                "label": "值改变时触发",
                "actionName": "change",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "评分",
        "itemType": "rate",
        "itemIcon": "rate",
        "category": 1,
        "fields": [
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "最大分值",
                "fieldName": "max",
                "fieldType": "number",
                "required": false,
                "selectValues": "5",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否显示辅助文字",
                "fieldName": "showText",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否允许半选",
                "fieldName": "allowHalf",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "图标颜色",
                "fieldName": "colors",
                "fieldType": "input",
                "required": false,
                "selectValues": ["#F7BA2A", "#F7BA2A", "#F7BA2A"],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "低分和中等分数的界限值",
                "fieldName": "lowThreshold",
                "fieldType": "number",
                "required": false,
                "selectValues": "2",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "只读时未选中 icon 的颜色",
                "fieldName": "disabledVoidColor",
                "fieldType": "input",
                "required": false,
                "selectValues": "#EFF2F7",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "辅助文字的颜色",
                "fieldName": "textColor",
                "fieldType": "color",
                "required": false,
                "selectValues": "#1F2D3D",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "高分和中等分数的界限值",
                "fieldName": "highThreshold",
                "fieldType": "number",
                "required": false,
                "selectValues": "4",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "图标颜色",
                "fieldName": "voidColor",
                "fieldType": "input",
                "required": false,
                "selectValues": "#C6D1DE",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "取色器",
        "itemType": "color",
        "itemIcon": "color",
        "category": 1,
        "fields": [
            {
                "label": "颜色的格式",
                "fieldName": "colorFormat",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "HSL", "value": "hsl"}, {"name": "HSV", "value": "hsv"}, {
                    "name": "HEX",
                    "value": "hex"
                }, {"name": "RGB", "value": "rgb"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否支持透明度选择",
                "fieldName": "showAlpha",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "否触发表单的校验",
                "fieldName": "validateEvent",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "预定义颜色",
                "fieldName": "predefine",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "下拉框的类名",
                "fieldName": "popperClass",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [
            {

                "label": "颜色发生改变时触发",
                "actionName": "activeChange",
                "fieldType": "button"
            },
            {

                "label": "数据改变时触发",
                "actionName": "change",
                "fieldType": "button"
            }
        ],
        "preps": []
    },
    {
        "itemName": "富文本编辑器",
        "itemType": "htmleditor",
        "itemIcon": "html-text",
        "category": 1,
        "fields": [
            {
                "label": "上传地址",
                "fieldName": "action",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "高度",
                "fieldName": "height",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "树形选择器",
        "itemType": "tselect",
        "itemIcon": "tselect",
        "category": 1,
        "fields": [
            {
                "label": "显示选择框",
                "fieldName": "showCheckbox",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "允许选择父节点",
                "fieldName": "checkStrictly",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "显示所有选中标签",
                "fieldName": "collapseTagsTooltip",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "多选时按文字形式展示",
                "fieldName": "collapseTags",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "是否多选",
                "fieldName": "multiple",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "配置选项",
                "fieldName": "props",
                "fieldType": "json",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "值类型",
                "fieldName": "valueType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "字符串", "value": "string"}, {"name": "数值", "value": "number"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "是否可筛选",
                "fieldName": "filterable",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "true",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "获得焦点弹出选项菜单",
                "fieldName": "automaticDropdown",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "获得焦点弹出选项菜单",
                "fieldName": "fitInputWidth",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否自动完成",
                "fieldName": "autocomplete",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否显示下拉箭头",
                "fieldName": "showArrow",
                "fieldType": "switch",
                "required": false,
                "selectValues": "",
                "defaultValues": "true",
                "category": 2,
                "configParams": []
            },
            {
                "label": "从服务器加载内容时显示的文本",
                "fieldName": "loadingText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "默认选择第一个匹配项",
                "fieldName": "defaultFirstOption",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义筛选方法",
                "fieldName": "filterMethod",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义类名",
                "fieldName": "popperClass",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "无匹配时显示的文字",
                "fieldName": "noMatchText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "Tooltip 主题",
                "fieldName": "effect",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "暗黑", "value": "dark"}, {"name": "高亮", "value": "light"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "标签类型",
                "fieldName": "placement",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "顶部", "value": "top"}, {
                    "name": "顶部开始",
                    "value": "top-start"
                }, {"name": "顶部结束", "value": "top-end"}, {"name": "底部", "value": "bottom"}, {
                    "name": "底部开始",
                    "value": "bottom-start"
                }, {"name": "底部结束", "value": "bottom-end"}, {"name": "左边", "value": "left"}, {
                    "name": "左上",
                    "value": "left-start"
                }, {"name": "左下", "value": "left-end"}, {"name": "右边", "value": "right"}, {
                    "name": "右上",
                    "value": "right-start"
                }, {"name": "右下", "value": "right-end"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "自定义远程搜索",
                "fieldName": "remoteMethod",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "无数据显示的文字",
                "fieldName": "noDataText",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否允许用户创建新条目",
                "fieldName": "allowCreate",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "唯一标识KEY",
                "fieldName": "valueKey",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最多可以选择的项目数",
                "fieldName": "multipleLimit",
                "fieldType": "number",
                "required": false,
                "selectValues": "0",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否保留搜索关键词",
                "fieldName": "reserveKeyword",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "否从服务器远程加载",
                "fieldName": "remote",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "是否正在从远程获取数据",
                "fieldName": "loading",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "标签类型",
                "fieldName": "tagType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "成功", "value": "success"}, {
                    "name": "信息",
                    "value": "info"
                }, {"name": "警告", "value": "warning"}, {"name": "危险", "value": "danger"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "Html组件",
        "itemType": "html",
        "itemIcon": "text",
        "category": 1,
        "fields": [
            {
                "label": "内容",
                "fieldName": "content",
                "fieldType": "button",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "静态文本",
        "itemType": "text",
        "itemIcon": "text",
        "category": 1,
        "fields": [
            {
                "label": "文本内容",
                "fieldName": "textContent",
                "fieldType": "input",
                "required": true,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "数据范围",
        "itemType": "number-range",
        "itemIcon": "text",
        "category": 1,
        "fields": [
            {
                "label": "数值精度",
                "fieldName": "precision",
                "fieldType": "number",
                "required": false,
                "selectValues": "0",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "计数器步长",
                "fieldName": "step",
                "fieldType": "input",
                "required": false,
                "selectValues": "1",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "是否使用控制按钮",
                "fieldName": "controls",
                "fieldType": "switch",
                "required": false,
                "selectValues": "[true,false]",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最小值",
                "fieldName": "min",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最大值",
                "fieldName": "max",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "清空时显示的值",
                "fieldName": "valueOnClear",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "最小值", "value": "min"}, {"name": "最大值", "value": "max"}],
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "占位信息",
                "fieldName": "placeholder",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "分割线",
        "itemType": "divider",
        "itemIcon": "text",
        "category": 1,
        "fields": [
            {
                "label": "内容",
                "fieldName": "content",
                "fieldType": "input",
                "required": false,
                "selectValues": " ",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "内容位置",
                "fieldName": "contentPosition",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "左", "value": "left"}, {"name": "中", "value": "center"}, {
                    "name": "右",
                    "value": "right"
                }],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "方向",
                "fieldName": "direction",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "水平", "value": "horizontal"}, {"name": "垂直", "value": "vertical"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "图标",
        "itemType": "icon",
        "itemIcon": "text",
        "category": 1,
        "fields": [
            {
                "label": "图标类型",
                "fieldName": "iconType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "系统图标", "value": "system"}, {"name": "用户自定义图标", "value": "user"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [],
        "selfFields": [],
        "actions": [],
        "preps": []
    },
    {
        "itemName": "图片",
        "itemType": "image",
        "itemIcon": "text",
        "category": 1,
        "fields": [
            {
                "label": "图片展示风格",
                "fieldName": "assignType",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "路径", "value": "url"}, {"name": "图片", "value": "image"}],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "适应类型",
                "fieldName": "fit",
                "fieldType": "select",
                "required": false,
                "selectValues": [{"name": "填充", "value": "fill"}, {
                    "name": "包含",
                    "value": "contain"
                }, {"name": "覆盖", "value": "cover"}, {"name": "无", "value": "none"}, {
                    "name": "向下缩放",
                    "value": "scale-down"
                }],
                "defaultValues": "",
                "category": 1,
                "configParams": []
            },
            {
                "label": "预览数据",
                "fieldName": "previewSrcList",
                "fieldType": "json",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 1,
                "configParams": []
            }
        ],
        "advancedFields": [
            {
                "label": "缩放速率",
                "fieldName": "zoomRate",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "图片高度",
                "fieldName": "height",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "图片宽度",
                "fieldName": "width",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最大缩放比例",
                "fieldName": "maxScale",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "最小缩放比例",
                "fieldName": "minScale",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "预览图像索引",
                "fieldName": "initialIndex",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            },
            {
                "label": "zIndex",
                "fieldName": "zIndex",
                "fieldType": "input",
                "required": false,
                "selectValues": "",
                "defaultValues": "",
                "category": 2,
                "configParams": []
            }
        ],
        "selfFields": [],
        "actions": [],
        "preps": []
    }
];
