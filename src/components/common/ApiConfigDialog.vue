...
import {type ApiConfig, protocolOptions, validateApiConfig} from "@/utils/apiConfig";
...
// 响应式数据
const apiConfigForm = reactive<ApiConfig>({...props.modelValue});
...
watch(() => props.modelValue, (newValue) => {
  Object.assign(apiConfigForm, newValue);
}, {deep: true});
...
const handleSave = () => {
  // 验证API配置
  const validation = validateApiConfig(apiConfigForm);
  if (!validation.valid) {
    error(validation.error || "API配置不完整");
    return;
  }

  // 发送更新事件
  emit("update:modelValue", {...apiConfigForm});
  emit("save", {...apiConfigForm});
...
const handleCancel = () => {
  // 恢复原始值
  Object.assign(apiConfigForm, props.modelValue);
  emit("update:visible", false);
};
  ...
  >
    ...
    >
      <el-form-item
          label="系统环境"
          prop="env"
          :rules="[{ required: true, message: '请选择系统环境', trigger: 'blur' }]"
      >
      ...
      >
        <el-select v-model="apiConfigForm.httpMethod" placeholder="请选择请求方式">
          <el-option
      ...
      <el-form-item
          label="IP/域名/服务名"
          prop="host"
          :rules="[{ required: true, message: '请输入IP/域名/服务名', trigger: 'blur' }]"
      >
        <el-input
