...
export class ChatSocketService {
    ...
    public initConnect(): Promise<boolean> {
        return new Promise((resolve, reject) => {
            try {
                console.log("Attempting to connect to chat WebSocket at:", this.options.endpoint!);
    ...
    public sendGroupMessage(message: Omit<ChatMessage, "type">): void {
        ...
        try {
            ...
            this.stompClient.send("/app/form/group-message", {}, JSON.stringify(fullMessage));
     ...
     * @param formId
     */
    public subscribeToPeerJoined(callback: (message: any) => void, formId: string): string | null {
        if (!this.stompClient || !this.connected) {
            console.error("Chat WebSocket not connected");
            return null;
        }
        try {
            console.log("Attempting to subscribe to peer joined notifications for user:", formId);
            const subscription = this.stompClient.subscribe(`/form/${formId}/peer-joined`, (message) => {
            ...
            console.log("Successfully subscribed to peer joined notifications for user:", formId);
     ...
     * @param formId
     */
    public subscribeToExistingPeers(callback: (message: any) => void, formId: string): string | null {
        if (!this.stompClient || !this.connected) {
        ...
        try {
            console.log("Attempting to subscribe to existing peers notifications for user:", formId);
            const subscription = this.stompClient.subscribe(`/form/${formId}/existing-peers`, (message) => {
            ...
            console.log("Successfully subscribed to existing peers notifications for user:", formId);
            return subscription.id;
        } catch (error) {
     ...
     * @param callback
     */
    public subscribeToGroupMessages(formId: string, callback: (message: ChatMessage) => void): string | null {
        if (!this.stompClient || !this.connected) {
            console.error("Chat WebSocket not connected");
            return null;
        }
        ...
        try {
            const subscription = this.stompClient.subscribe(`/form/${formId}/message`, (message) => {
                const data = JSON.parse(message.body);
                callback(data);
            });

            console.log(`Subscribed to group messages for group ${formId}`);
            return subscription.id;
        } catch (error) {
