...
var noImplicitEnd = function () {
    ...
    function isImplicitEnd(node) {
        ...
        if (is$7(node, 'bpmn:IntermediateThrowEvent') && isLinkEvent(node)) {
            return false;
        }

        if (is$7(node, 'bpmn:EndEvent')) {
            return false;
        }
...
var noImplicitStart = function () {
    ...
    function isLinkEvent(node) {
        ...
        return eventDefinitions.length && eventDefinitions.every(
            definition => is$6(definition, 'bpmn:LinkEventDefinition')
        );
    }
    ...
    function isImplicitStart(node) {
        ...
        if (is$6(node, 'bpmn:IntermediateCatchEvent') && isLinkEvent(node)) {
            return false;
        }

        if (isAny$1(node, ['bpmn:StartEvent', 'bpmn:BoundaryEvent'])) {
            return false;
        }
    ...
    function check(node, reporter) {
        ...
        if (!isAny$1(node, ['bpmn:Event', 'bpmn:Activity', 'bpmn:Gateway'])) {
            return;
        }
...
var noOverlappingElements = function () {
    ...
    function check(node, reporter) {
        ...
        rootElements
            .filter(element => is$5(element, 'bpmn:Collaboration'))
        ...
        rootElements
            .filter(element => is$5(element, 'bpmn:Process'))
            .forEach(process => {
...
function checkProcess(node, elementsToReport, elementsOutsideToReport, diObjects, parentDi) {
    ...
    flowElements.filter(element => !is$5(element, 'bpmn:DataStoreReference')).forEach(element => {
    ...
    // check subprocesses
    const subProcesses = flowElements.filter(element => is$5(element, 'bpmn:SubProcess'));
...
function isValidShapeElement(bounds) {
    return !!bounds && is$5(bounds, 'dc:Bounds') &&
