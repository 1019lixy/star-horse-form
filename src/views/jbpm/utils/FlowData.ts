        ...
        if (nodeConfig.conditionNodes) {
            ...
            nodeConfig.conditionNodes.forEach((row: any, index: number) => {
                if (row.childNode && modeler) {
                    ...
                    modeling.removeElements([row.element, ...row.childNode.element.outgoing, ...row.childNode.element.incoming]);
                    row.element = markRaw(modeling.connect(nodeConfig.element, row.childNode.element, {}));
...
const setDiagramType = (modeler: BpmnModeler) => {
    ...
    if (!process.businessObject.extensionElements) {
        ...
        if (!hasDiagramType) {
            modeling.updateModdleProperties(process.businessObject.extensionElements, properties, {
                values: [
