...
const addSubForm = (params: any) => {
  router.push({
    ...
    query: { parentId: params.idDynamicForm },
  });
};
const dataFormat = (name: string, cellValue: any, row: any): any => {
  if (name == 'datasourceConfigId') {
    return (
      dataSource.value.find((item) => item.value!.toString() == cellValue)
        ?.name || cellValue
    );
  }
  if (name == 'sysId') {
...
const dataChange = async (param: any) => {
  let { data, error } = await loadData(
    dataUrl.loadByIdUrl + '/' + param[primaryKey],
    {},
  );
  if (error) {
    warning(error);
                ...
                >
                  <template v-for="(item, index) in viewBtnList">
                    <el-menu-item
                      v-if="
                        item.authority == 'none' ||
                        dynamicFormRef.permissionList()[item.authority]
                      "
                      :index="'1_' + index"
