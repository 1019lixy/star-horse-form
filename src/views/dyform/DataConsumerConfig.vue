<script setup lang="ts" name="DataConsumerConfig">
  ...
  const loadColumnFields = async () => {
    ...
    columnList.value = [];
    await postRequest("/userdb-manage/userdb/dynamicFormConsumerConfig/previewViewColumns", createMergeData()).then(
      (res) => {
        let redata = res.data;
        if (redata.code != 0) {
          error(redata.cnMessage);
  ...
  const dataList = (currentPage: number, pageSize: number) => {
    ...
    postRequest("/userdb-manage/userdb/dynamicFormConsumerConfig/preview", dataPo)
      .then((res) => {
        let redata = res.data;
        if (redata.code != 0) {
          error(redata.cnMessage);
  ...
  const lineOperation = (data: any) => {
    // console.log(data);
    let fdata = relationConditionList.value.find(
      (item) =>
        item.from == data.from && item.fromPort == data.fromPort && item.to == data.to && item.toPort == data.toPort
    );
    if (fdata) {
...
<template>
  ...
  <star-horse-dialog
    :dialogVisible="relationDialogVisible"
    :title="'关系配置'"
    :isBatch="false"
  ...
  >
    <star-horse-form
      :outerFormData="formConditionProps"
  ...
  <star-horse-dialog
    :dialogVisible="consumerDialogVisible"
    :title="'消费配置'"
    :isBatch="false"
    @merge="submitValid"
    @closeAction="closeAction"
  ...
  <el-card class="inner_content" style="height: 100%; padding: 5px">
    <star-horse-design
      ...
      :register-node="consumerNodeData"
      ref="containerDiagramRef"
      :tableFlag="true"
      :activeCollapse="'db'"
      :panelStyle="'other'"
      :showDbList="true"
