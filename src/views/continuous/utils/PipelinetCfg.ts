...
export const loadPipelineCfg = (idPipelineInfo: string) => {
  return loadData(
    `${execRecordApi.basePrefix}/loadConfigInfoById/${idPipelineInfo}`,
    {},
  );
};
/**
...
export const loadPipelineInstance = (idPipelineInfo: string) => {
  return loadData(
    `${execUrl.basePrefix}/loadInstanceById/${idPipelineInfo}`,
    {},
  );
};
/**
...
export const execLine = (idPipelineConfig: string, methodName: string) => {
  postRequest(
    execUrl.basePrefix + `/${methodName}/` + idPipelineConfig,
    {},
  ).then((res) => {
    if (res?.data?.code == 0) {
      success("执行成功");
...
export const execNode = (item: any, instance: any, methodName: string) => {
  ...
  postRequest(
    execUrl.basePrefix +
      `/${methodName}/${instance.idPipelineInstance}/` +
      item.idNodeInfo,
    {},
  ).then((res) => {
    if (res?.data?.code == 0) {
