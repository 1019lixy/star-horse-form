<template>
  <el-drawer v-model="drawer" title="添加数据" @close="closeAction">
    <star-horse-form-list
      :fieldList="fieldList"
      v-model:dataForm="extendForm"
      :batch-name="currentName"
    />
  </el-drawer>
    ...
    <el-tabs v-model="noticeTab">
      ...
      <el-tab-pane key="2" name="2" label="内容设置">
          ...
          <el-checkbox-group v-model="node.noticeType">
            <el-row :gutter="24">
              <el-col :span="8" v-for="(notice, i) in notices" :key="i">
                <el-checkbox :value="notice.value"
                  >{{ notice.name }}
                </el-checkbox>
              </el-col>
            </el-row>
        ...
        <el-form-item
          label="外部手机号"
          prop="extendPhones"
          v-if="node.noticeType.includes('shortMsg')"
        >
          <el-badge
            :value="node.extendPhones?.length || 0"
            class="item"
            type="primary"
          >
            <el-button
        ...
        <el-form-item
          label="外部邮箱账号"
          prop="extendEmails"
          v-if="node.noticeType.includes('email')"
        >
          <el-badge
            :value="node.extendEmails?.length || 0"
            class="item"
            type="primary"
          >
            <el-button
  ...
  <DrawerFooter @save="onClose" @close="onClose" />
</template>
...
const closeAction = () => {
  drawer.value = false;
  node.value[currentName.value] = extendForm.value[currentName.value].map(
    (item: any) => item.email || item.phone,
  );
};
