...
<template>
  ...
  <star-horse-dialog :dialogVisible="formAuthorityVisible" @merge="editFormAutority" @closeAction="closeAction"
                     :selfFunc="true" :compSize="compSize"
                     :title="'设置表单权限'" :source="3">
  ...
  <template v-if="dataList?.length > 0">
    ...
    <div class="flex mx-[10px] items-center w-full justify-center">
      ...
      <el-button @click="formAuthority" v-if="node.type!=FlowNodeEnums.APPLY_NODE" icon="setting" type="info">表单权限配置
      </el-button>
    </div>
  </template>
