...
import NodeWrap from "@/views/workflow/dingDingStyle/nodeWrap.vue";
import StarHorseIcon from "@/components/comp/StarHorseIcon.vue";

let isTried = ref<boolean>(false);
let tipList = ref<Array<any>>([]);
let nodeConfig = ref<any>({});
let dataFields = ref<Array<any>>([
  {name: "日期", key: "date", type: "date"},
  {name: "地址", key: "adress"},
  {name: "状态", key: "state"}
]);
let nowVal = ref<number>(100);
let flowPermission = ref<Array<any>>([]);
let tableId = ref<string>("");
let nodeConfigss = ref<any>({
  "type": 0, //类型
  "priorityLevel": "", //优先级
  "settype": "", //审批人类型
  "selectMode": "", // 1人还是多人
...
const reErr = (data) => {
  if (data.childNode) {
    if (data.childNode.type == 1) {
      ...
      reErr(data.childNode.childNode)
      ...
      for (
        ...
        reErr(data.childNode.conditionNodes[i])
      }
    }
  } else {
    data.childNode = null
...
const handleData = (data) => {
  if (data.childNode) {
    ...
    handleData(data.childNode);
  }
  return data;
}
const workflowSave = () => {
  // 调接口存数据
  let data = handleData(nodeConfig);
    ...
    <div class="dingflow-design">
      <div class="zoom flex">
        <div :class="'zoom-out'+ (nowVal==50?' disabled':'')" @click="zoomSize(1)">
          <star-horse-icon icon-class="minus"/>
        </div>
        <span>{{ nowVal }}%</span>
      ...
      <div class="box-scale" id="box-scale"
        ...
        <nodeWrap v-model:nodeConfig="nodeConfig" :dataFields="dataFields" :isTried="isTried"></nodeWrap>
        <div class="end-node flex">
          <p>结束</p>
        </div>
      </div>
